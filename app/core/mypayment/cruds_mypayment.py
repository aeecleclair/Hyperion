...
from app.core.mypayment import models_mypayment, schemas_mypayment, types_mypayment
...
async def get_requests_by_wallet_id(
    ...
    result = await db.execute(
        ...
        .where(
            models_mypayment.Request.status == types_mypayment.RequestStatus.PROPOSED
            if not include_used
            else and_(True),
        ),
    )
...
async def mark_expired_requests_as_expired(
    ...
    await db.execute(
        ...
        .where(
            models_mypayment.Request.status == schemas_mypayment.RequestStatus.PROPOSED,
            models_mypayment.Request.creation + timedelta(minutes=REQUEST_EXPIRATION)
            <= datetime.now(tz=UTC),
        )
        .values(status=schemas_mypayment.RequestStatus.EXPIRED),
    )
