...
async def store_scan_qrcode(
    ...
    **The user must have the `can_bank` permission for this store**
    ...
    already_existing_used_qrcode = await cruds_myeclpay.get_used_transaction_request(
    ...
    await cruds_myeclpay.create_used_transaction_request(
    ...
    # after roleback the used QR Code will still be created and committed in db.
...
@router.post(
    "/myeclpay/stores/{store_id}/purchase",
    response_model=standard_responses.Result,
    status_code=200,
)
async def user_purchase_store(
    store_id: UUID,
    purchase_info: schemas_myeclpay.PurchaseInfo,
    db: AsyncSession = Depends(get_db),
    user: CoreUser = Depends(is_user_an_ecl_member),
    request_id: str = Depends(get_request_id),
    notification_tool: NotificationTool = Depends(get_notification_tool),
    ...
    **The user must be authenticated to use this endpoint**
    ...
    already_existing_used_payment = await cruds_myeclpay.get_used_transaction_request(
    ...
    async with db.begin_nested():
        ...
        if debited_wallet.user.id != user.id:
            ...
            raise HTTPException(
                status_code=400,
                detail="MyECLPay: Mismatch between the user who sent the request and the user owning the signatory device",
            )
