...
async def open_vote(
    ...
    **The user must be a member of the group CAA to use this endpoint**
    ...
    lists = await cruds_campaign.get_lists(db=db)
    with open(
        f"data/campaigns/lists-{datetime.now(settings.TIMEZONE).isoformat(sep='-',timespec='minutes').replace(':','-')}.json",
        "w",
    ) as file:
...
async def reset_vote(
    ...
    **The user must be a member of the group CAA to use this endpoint**
    ...
    try:
        ...
        with open(
            f"data/campaigns/results-{datetime.now(settings.TIMEZONE).isoformat(sep='-',timespec='minutes').replace(':','-')}.json",
            "w",
        ) as file:
            json.dump(
